...
\begin{abstract}
We give an optimal lower bound on principal curvatures of immersed $n$-torus in a Euclidean ball of large dimension. 
\end{abstract}

\section{Introduction}

This note is inspired by examples of isometric embeddings of $n$-torus $\TT^n$ into the unit ball $\BB^q$ of $q$-dimensional Euclidean space for all large $q$ such that all it principal curvatures identically equal to $\sqrt{3\cdot n/(n+2)}$ and the induced metric is flat.
...
The following theorem implies that the bound $\sqrt{3\cdot n/(n+2)}$ is optimal.

\begin{thm}{Theorem}
\label{thm:main}
Suppose $\TT^n$ is smoothly immersed in $\BB^q$.
Then its maximal normal curvature is at least $\sqrt{3\cdot n/(n+2)}$.
\end{thm}
...
(The letter $\zh$ suppose to remind that it is squared $K$.)
...
Is it true that for any smooth immersion $\TT^n\looparrowright\BB^q$ we have $\zh\ge3\cdot \tfrac{n}{n+2}$ at some point? 
\end{thm}
...
the scalar curvature, the mean curvature vector.
...
and the unit ball centered at the origin of the $q$-dimensional Euclidean space $\RR^q$ is denoted by $\BB^q$.

The following lemma was essentially proved by István Fáry \cite{fary,tabachnikov}.
...
\parit{Proof.}
Consider the function $u\:p\mapsto \tfrac12\cdot |\iota(p)|^2$ on $L$.
Note that
...
Let $L$ be a manifold smoothly immersed in $\BB^q$, and its normal curvatures are at most $2$.
...
extend $\sigma$ after $x$ by a unit-speed semicircle $\tilde\gamma$ with curvature $2$ in such a way that the concatenation $\sigma*\tilde\gamma$ is is an arc of a smooth convex plane curve; see the figure.
...
Here is the well-known formula for the scalar curvature of the metric $u^{\frac{4}{n-2}}\cdot g$:
\[\left(\Sc\cdot u-4{\cdot}\tfrac{n-1}{n-2}{\cdot}\Delta u\right)\cdot u^{\frac{n-2}{n+2}};\]
see for example \cite[6.3]{aubin}.
Recall that \emph{any Riemannian metric $g$ on $\TT^n$ has nonpositive scalar curvature at some point} \cite[Cor. A]{gromov-lawson}.
...
This claim plays a central role in the following proof.
...
\parit{Proof.}
Choose a point $x$ on the manifold, say $L$.
Further we will identify $\T_x$ and $\T_x^*$ using the Riemannian metric.
Let us denote by $\T_x^n$ the $n^{\text{th}}$ tensor power of $\T_x$.
Denote by $\S^n(\T_x)$ and $\Lambda^n(\T_x)$ respectively subspace of symmetric and antisymmetric
elements of $\T_x^n$.

Let $\II$ be the second fundamental form at $x$.
Consider the \emph{extrinsic curvature tensor}
$$\Phi (X,Y,Z,W)\df \langle \II(X,Y),\II(Z,W)\rangle.$$
Note that $\Phi\in\S^2(\S^2(\T_x))$.
Observe that $\Phi$ remembers the $\II$ up to rotation of $\N_x$.
Further, $|H|^2$, $\zh$ and $\Sc$ in the Gauss formula depend linearly on $\Phi$ and they are invariant with respect to the action of orthogonal group on $\T_x$.
Therefore we can assume that $\Phi$ is symmetric with respect to the $\mathrm{O}(n)$-action on $\T_x$
...
(\Phi(X,Y,Z,W)+\Phi(Y,Z,X,W)+\Phi(Z,X,Y,W))\in \S^4(\T_x),$$
and
$$\Rm(X,Y,Z,W)=\Phi(X,Z,Y,W)-\Phi(X,W,Y,Z)$$
is the Riemannian curvature tensor of $L$.

Since $\Phi$ is invariant with respect to the $\mathrm{O}(n)$-action on $\T_x$,
$\Rm$ and $E$ are symmetric as well.
Therefore $\Rm$ is completely determined by $\Sc$ and $E$ is determined by $\zh$;
in fact $E(X,X,X,X)\equiv\zh\cdot|X|^4$.


Tensor $E$ represents the \emph{purely extrinsic} part $\Phi$.
In some sense, $E$ is a higher order analog of the metric tensor.
It measures wrinkling of the embedding --- the more it is wrinkled the bigger $E$ gets.
Note that
\[E(X,X,X,X)=|\II(X,X)|^2\]
is homogeneous polynomial of degree $4$ on $\T_x$ 
...
It follows that passing averaging $\Phi$ along the action of orthogonal groups on $\T_x$ does not change the quantities in the claim.
...
elements of $\T^n$.
The scalar product on $\T$ induces a
...
Let $L\subset\RR^q$ be a smooth submanifold and $s_x\:\S^2(\T_xL)\to \N_x L$ its second fundamental form at $x\in L$;
here $\T L$ and $\N  L$ are tangent and normal bundles over $L$ respectively.
Consider the \emph{extrinsic curvature tensor}
$$\Phi (X,Y,Z,W)\df \langle s(X,Y),s(Z,W)\rangle,$$
here $\Phi$ is a section of $\S^2(\S^2(\T L))$.
...
Observe that 
\begin{itemize}
 \item The extrinsic curvature tensor $\Phi$ remembers the second fundamental form up to action of orthogonal group on $\N$.
 \item The quantities $|H|^2$, $\zh$ and $\Sc$ in the Gauss formula depend linearly on $\Phi$ and they are invariant with respect to the action of orthogonal group on $\T$.
\end{itemize}
