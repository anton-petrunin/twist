...
Denote by $H(p)$, $\zh(p)$, and $\Sc(p)$ the mean curvature vector, the average value of $|\II(\vec u,\vec u)|^2$, and the scalar curvature of $L$ at $p$ respectively. 
...
Playing a bit with the formulas above seems to give an asymptotic lower bound $\sqrt{8/3}$; it is quite close to $\sqrt{3}$ and can be improved a bit further, but the optimal bound requires an extra idea.
